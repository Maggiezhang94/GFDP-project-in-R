...
df3.original <- df3
df3.copy <- df3; df3.copy$orig.industry_name <- "all"
df3 <- rbind.data.frame(df3, df3.copy)
...
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(
      selectInput("group", "Choose the group", 
                  choices = c(names(table(df3$orig.industry_name)))),
      htmlOutput("Summary")
    ),
    
    mainPanel(
      tabsetPanel(
        tabPanel("Corrplot.Pearson",
                 plotOutput("Corrplot.Pearson"), 
                 plotOutput("pearson.node")),
        tabPanel("Corrplot.Spearman",
                 plotOutput("Corrplot.Spearman"),
                 plotOutput("spearman.node"))
      ))
  ))
...
server <- function(input, output){
  output$Corrplot.Pearson <- renderPlot({ 
    use.one <- subset(df3, orig.industry_name %in% c(input$group)) %>% dplyr::select(employees:age, growth_tot_contrib_pens:global.rank,country.rank, web.creation.year, log.rank)
    use.one <- use.one[complete.cases(use.one),]
    M <- cor(use.one)
    ...
    #cor(use.one, use = "complete.obs") %>% corrplot.mixed()
  })
  
  output$Corrplot.Spearman <- renderPlot({ 
    use.one <- subset(df3, orig.industry_name %in% c(input$group)) %>% dplyr::select(employees:age, growth_tot_contrib_pens:global.rank,country.rank, web.creation.year, log.rank)
    use.one <- use.one[complete.cases(use.one),]
    M <- cor(use.one, method = "spearman")
  ...
  output$pearson.node <- renderPlot({
    corr.data.using <- subset(df3, orig.industry_name %in% c(input$group)) %>% dplyr::select(employees:age, growth_tot_contrib_pens:global.rank,country.rank, web.creation.year, log.rank)
    cor.pearson.circle <- cor(corr.data.using, method = "pearson", use = "complete.obs")
    qgraph(cor.pearson.circle, shape = "circle", posCol = "darkgreen", negCol = "darkred", layout = "spring", vsize = 10,  title = "Pearson Correlation W/ Nodes All")
    names(corr.data.using)})
  
  output$spearman.node <- renderPlot({
    corr.data.using <- subset(df3, orig.industry_name %in% c(input$group)) %>% dplyr::select(employees:age, growth_tot_contrib_pens:global.rank,country.rank, web.creation.year, log.rank)
    cor.spearman.circle <- cor(corr.data.using, method = "spearman", use = "complete.obs")
    qgraph(cor.spearman.circle, shape = "circle", posCol = "darkgreen", negCol = "darkred", layout = "spring", vsize = 10,  title = "Spearman Correlation W/ Nodes All")
    names(corr.data.using)})
  
  output$Summary <- renderPrint({
    use.one <- subset(df3, orig.industry_name %in% c(input$group)) %>% dplyr::select(employees:age, growth_tot_contrib_pens:global.rank,country.rank, web.creation.year, log.rank)
    print(paste("Your correlation includes", dim(use.one)[1], "observations"))
  })
